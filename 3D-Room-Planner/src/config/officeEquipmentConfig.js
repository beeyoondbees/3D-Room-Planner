const officeEquipmentConfig = {
    modelPaths: {},
    dimensions: {},
    catalog: {
      "Tables": {
        "Conference Table": [
          {
            id: "con-table1",
            name: "Modern Wooden Table",
            icon: "/assets/icons/products/con-table1.png"
          },
          {
            id: "con-table2",
            name: "Sleek Design Coffee Table",
            icon: "/assets/icons/products/con-table2.png"
          },
          {
            id: "con-table3",
            name: "Minimalist White Table",
            icon: "/assets/icons/products/con-table3.png"
          },
          {
            id: "con-table4",
            name: "Rounded Edge Office Table",
            icon: "/assets/icons/products/con-table4.png"
          },
          {
            id: "con-table5",
            name: "Large White Conference Table",
            icon: "/assets/icons/products/con-table5.png"
          }
        ],
        "Office Tables": [
          {
            id: "off-table1",
            name: "White Corner Desk",
            icon: "/assets/icons/products/off-table1.png"
          },
          {
            id: "off-table3",
            name: "Modern Conference Table",
            icon: "/assets/icons/products/off-table3.png"
          },
          {
            id: "off-table4",
            name: "Minimalist White Desk",
            icon: "/assets/icons/products/off-table4.png"
          },
          {
            id: "off-table5",
            name: "Wooden Meeting Table",
            icon: "/assets/icons/products/off-table5.png"
          },
          {
            id: "off-table6",
            name: "Sleek Black Desk",
            icon: "/assets/icons/products/off-table6.png"
          },
          {
            id: "off-table7",
            name: "Wooden Executive Desk",
            icon: "/assets/icons/products/off-table7.png"
          }
        ],
        "Home Tables": [
          {
            id: "home-table1",
            name: "Modern Side Table",
            icon: "/assets/icons/products/home-table1.png"
          },
          {
            id: "home-table2",
            name: "Contemporary Executive Desk",
            icon: "/assets/icons/products/home-table2.png"
          },
          {
            id: "home-table3",
            name: "Minimalist Black Desk",
            icon: "/assets/icons/products/home-table3.png"
          },
          {
            id: "home-table4",
            name: "Sleek TV Stand",
            icon: "/assets/icons/products/home-table4.png"
          },
          {
            id: "home-table5",
            name: "Round Wooden Coffee Table",
            icon: "/assets/icons/products/home-table5.png"
          },
          {
            id: "home-table6",
            name: "Vintage White Pedestal Table",
            icon: "/assets/icons/products/home-table6.png"
          },
          {
            id: "home-table7",
            name: "Wooden Outdoor Dining Table",
            icon: "/assets/icons/products/home-table7.png"
          },
          {
            id: "home-table8",
            name: "White and Wood Dining Table",
            icon: "/assets/icons/products/home-table8.png"
          }
        ]
      },
      "Chairs": {
        "Office Chairs": [
          {
            id: "off-chair1",
            name: "Executive Ergonomic Chair",
            icon: "/assets/icons/products/off-chair1.png"
          },
          {
            id: "off-chair2",
            name: "Modern Office Task Chair",
            icon: "/assets/icons/products/off-chair2.png"
          },
          {
            id: "off-chair3",
            name: "Basic Office Chair",
            icon: "/assets/icons/products/off-chair3.png"
          },
          {
            id: "off-chair4",
            name: "Adjustable White Desk Chair",
            icon: "/assets/icons/products/off-chair4.png"
          },
          {
            id: "off-chair5",
            name: "Sleek Black Mesh Chair",
            icon: "/assets/icons/products/off-chair5.png"
          },
          {
            id: "off-chair6",
            name: "Professional Blue Office Chair",
            icon: "/assets/icons/products/off-chair6.png"
          },
          {
            id: "off-chair7",
            name: "Compact Black Swivel Chair",
            icon: "/assets/icons/products/off-chair7.png"
          },
          {
            id: "off-chair8",
            name: "Minimalist Office Chair",
            icon: "/assets/icons/products/off-chair8.png"
          }
        ],
        "Meeting Chairs": [
          {
            id: "meet-chair1",
            name: "Vintage Wooden Dining Chair",
            icon: "/assets/icons/products/meet-chair1.png"
          },
          {
            id: "meet-chair2",
            name: "Contemporary Office Task Chair",
            icon: "/assets/icons/products/meet-chair2.png"
          },
          {
            id: "meet-chair3",
            name: "Modern Minimalist Guest Chair",
            icon: "/assets/icons/products/meet-chair3.png"
          }
        ],
        "Home Chairs": [
          {
            id: "home-chair1",
            name: "Scandinavian Wood Dining Chair",
            icon: "/assets/icons/products/home-chair1.png"
          },
          {
            id: "home-chair2",
            name: "Classic Wooden Dining Chair",
            icon: "/assets/icons/products/home-chair2.png"
          },
          {
            id: "home-chair3",
            name: "Modern White Armchair",
            icon: "/assets/icons/products/home-chair3.png"
          },
          {
            id: "home-chair4",
            name: "Vintage Fabric Armchair",
            icon: "/assets/icons/products/home-chair4.png"
          },
          {
            id: "home-chair5",
            name: "Foldable Metal Chair",
            icon: "/assets/icons/products/home-chair5.png"
          }
        ]
      },
      "BEDS": {
        "BEDS": [
          {
            id: "BED_1",
            name: "Single Wooden Bed",
            icon: "/assets/icons/products/BED_1.png"
          },
          {
            id: "BED_2",
            name: "Rustic Wooden Single Bed",
            icon: "/assets/icons/products/BED_2.png"
          },
          {
            id: "BED_3",
            name: "Modern Storage Bed",
            icon: "/assets/icons/products/BED_3.png"
          },
          {
            id: "BED_4",
            name: "Mid-Century Modern Bed",
            icon: "/assets/icons/products/BED_4.png"
          },
          {
            id: "BED_5",
            name: "Upholstered Bed",
            icon: "/assets/icons/products/BED_5.png"
          },
          {
            id: "BED_6",
            name: "Classic Wooden Bed",
            icon: "/assets/icons/products/BED_6.png"
          },
          {
            id: "BED_7",
            name: "Antique Carved Wooden Bed",
            icon: "/assets/icons/products/BED_7.png"
          }
        ],
      },
      "SOBA": {
        "SOBA": [
          {
            id: "SOBA_1",
            name: "Modern Fabric Sofa",
            icon: "/assets/icons/products/SOBA_1.png"
          },
          {
            id: "SOBA_2",
            name: "Large Sectional Sofa",
            icon: "/assets/icons/products/SOBA_2.png"
          },
          {
            id: "SOBA_3",
            name: "Classic Fabric Sofa",
            icon: "/assets/icons/products/SOBA_3.png"
          },
          {
            id: "SOBA_4",
            name: "Retro Futon Sofa Bed",
            icon: "/assets/icons/products/SOBA_4.png"
          },
          {
            id: "SOBA_5",
            name: "Mid-Century Leather Sofa",
            icon: "/assets/icons/products/SOBA_5.png"
          },
          {
            id: "SOBA_6",
            name: "Contemporary Low-Profile Sofa",
            icon: "/assets/icons/products/SOBA_6.png"
          },
          {
            id: "SOBA_7",
            name: "Chesterfield Leather Sofa",
            icon: "/assets/icons/products/SOBA_7.png"
          },
          {
            id: "SOBA_8",
            name: "Minimalist Slipcover Sofa",
            icon: "/assets/icons/products/SOBA_8.png"
          }
        ],
      },
      "Cabinets and Containers": {
        "Filing Cabinets": [
          {
            id: "wooden-cab1",
            name: "4-Drawer Filing Cabinet",
            icon: "/assets/icons/products/wooden-cab1.png"
          },
          {
            id: "wooden-cab2",
            name: "Lateral File Cabinet",
            icon: "/assets/icons/products/wooden-cab2.png"
          },
          {
            id: "wooden-cab3",
            name: "Mobile Pedestal",
            icon: "/assets/icons/products/wooden-cab3.png"
          },
          {
            id: "wooden-cab4",
            name: "Vertical File Cabinet",
            icon: "/assets/icons/products/wooden-cab4.png"
          },
          {
            id: "wooden-cab5",
            name: "Mobile Pedestal",
            icon: "/assets/icons/products/wooden-cab5.png"
          },
          {
            id: "wooden-cab6",
            name: "Vertical File Cabinet",
            icon: "/assets/icons/products/wooden-cab6.png"
          },
          {
            id: "wooden-cab7",
            name: "Mobile Pedestal",
            icon: "/assets/icons/products/wooden-cab7.png"
          }
        ],
        "Color Cabinets": [
          {
            id: "cabinet1",
            name: "Bookshelf",
            icon: "/assets/icons/products/cabinet1.png"
          },
          {
            id: "cabinet2",
            name: "Storage Unit",
            icon: "/assets/icons/products/cabinet2.png"
          },
          {
            id: "cabinet3",
            name: "Display Cabinet",
            icon: "/assets/icons/products/cabinet3.png"
          },
          {
            id: "cabinet5",
            name: "Display Cabinet",
            icon: "/assets/icons/products/cabinet5.png"
          },
          {
            id: "cabinet_col6",
            name: "Office Wardrobe",
            icon: "/assets/icons/products/cabinet6.png"
          }
        ],
        "Cabinets": [
          {
            id: "cabinet_col1",
            name: "Bookshelf",
            icon: "/assets/icons/products/cabinet_col1.png"
          },
          {
            id: "cabinet_col2",
            name: "Storage Unit",
            icon: "/assets/icons/products/cabinet_col2.png"
          },
          {
            id: "cabinet_col3",
            name: "Display Cabinet",
            icon: "/assets/icons/products/cabinet_col3.png"
          },
          {
            id: "cabinet_col4",
            name: "Office Wardrobe",
            icon: "/assets/icons/products/cabinet_col4.png"
          },
          {
            id: "cabinet_col5",
            name: "Display Cabinet",
            icon: "/assets/icons/products/cabinet_col5.png"
          }
        ]
      },
      "Meeting Point": {
        "Conference Solutions": [
          {
            id: "con_meeting_4",
            name: "Video Conference Setup",
            icon: "/assets/icons/products/con_meeting_1.png"
          },
          {
            id: "con_meeting_1",
            name: "Whiteboard",
            icon: "/assets/icons/products/con_meeting_2.png"
          },
          {
            id: "con_meeting_2",
            name: "Flipchart",
            icon: "/assets/icons/products/con_meeting_3.png"
          },
          {
            id: "con_meeting_3",
            name: "Smart Board",
            icon: "/assets/icons/products/con_meeting_4.png"
          }
        ],
        "Presentation Equipment": [
          {
            id: "Pre_Equi_1",
            name: "Projector Stand",
            icon: "/assets/icons/products/Pre_Equi_1.png"
          },
          {
            id: "Pre_Equi_2",
            name: "Presentation Screen",
            icon: "/assets/icons/products/Pre_Equi_2.png"
          },
          {
            id: "Pre_Equi_3",
            name: "Lectern",
            icon: "/assets/icons/products/Pre_Equi_3.png"
          },
          {
            id: "Pre_Equi_4",
            name: "Mobile Presentation Cart",
            icon: "/assets/icons/products/Pre_Equi_4.png"
          }
        ]
      },
      "Computer Units": {
        "Computers & Monitors": [
          {
            id: "tech_con_1",
            name: "Desktop Computer",
            icon: "/assets/icons/products/tech_con_1.png"
          },
          {
            id: "tech_con_2",
            name: "Monitor Stand",
            icon: "/assets/icons/products/tech_con_2.png"
          },
          {
            id: "tech_con_3",
            name: "Dual Monitor Setup",
            icon: "/assets/icons/products/tech_con_3.png"
          },
          {
            id: "tech_con_4",
            name: "Laptop Stand",
            icon: "/assets/icons/products/tech_con_4.png"
          },
          {
            id: "tech_con_5",
            name: "Keyboard Tray",
            icon: "/assets/icons/products/tech_con_5.png"
          },
          {
            id: "tech_con_6",
            name: "Keyboard Tray",
            icon: "/assets/icons/products/tech_con_6.png"
          },
          {
            id: "tech_con_7",
            name: "Keyboard Tray",
            icon: "/assets/icons/products/tech_con_7.png"
          }
        ],
      },
      "TV Units": {
        "TV & Monitors": [
          {
            id: "TV_1",
            name: "Modern Low TV Unit",
            icon: "/assets/icons/products/TV_1.png"
          },
          {
            id: "TV_2",
            name: "Large Entertainment Wall Unit",
            icon: "/assets/icons/products/TV_2.png"
          },
          {
            id: "TV_3",
            name: "Contemporary TV Wall Panel",
            icon: "/assets/icons/products/TV_3.png"
          },
          {
            id: "TV_4",
            name: "Floating TV Unit with Shelves",
            icon: "/assets/icons/products/TV_4.png"
          },
          {
            id: "TV_5",
            name: "Wood and Glass TV Panel",
            icon: "/assets/icons/products/TV_5.png"
          },
          {
            id: "TV_6",
            name: "Retro Wooden TV Stand",
            icon: "/assets/icons/products/TV_6.png"
          },
          {
            id: "TV_7",
            name: "Classic TV Cabinet",
            icon: "/assets/icons/products/TV_7.png"
          }
        ],
      },
      "MATS": {
        "MATS": [
          {
            id: "MAT_1",
            name: "Brown Velvet Area Rug",
            icon: "/assets/icons/products/MAT_1.png"
          },
          {
            id: "MAT_2",
            name: "Beige Textured Carpet Rug",
            icon: "/assets/icons/products/MAT_2.png"
          },
          {
            id: "MAT_3",
            name: "Traditional Persian Red Rug",
            icon: "/assets/icons/products/MAT_3.png"
          },
          {
            id: "MAT_4",
            name: "Vintage Oriental Cream Rug",
            icon: "/assets/icons/products/MAT_4.png"
          },
          {
            id: "MAT_5",
            name: "Prayer Mat (Islamic Design",
            icon: "/assets/icons/products/MAT_5.png"
          },
          {
            id: "MAT_6",
            name: "Natural Jute Woven Mat",
            icon: "/assets/icons/products/MAT_6.png"
          }
        ],
      },
      "Audio Units": {
      "Audio Visual": [
          {
            id: "tech_aud_1",
            name: "Bookshelf Speaker on Stand",
            icon: "/assets/icons/products/tech_aud_1.png"
          },
          {
            id: "tech_aud_2",
            name: "Floorstanding Tower Speaker ",
            icon: "/assets/icons/products/tech_aud_2.png"
          },
          {
            id: "tech_aud_3",
            name: "Pair of Floor Speakers",
            icon: "/assets/icons/products/tech_aud_3.png"
          },
          {
            id: "tech_aud_4",
            name: "Microphone on Stand",
            icon: "/assets/icons/products/tech_aud_4.png"
          },
          {
            id: "tech_aud_5",
            name: "Studio Monitor Speaker",
            icon: "/assets/icons/products/tech_aud_5.png"
          },
          // {
          //   id: "tech_aud_6",
          //   name: "Monitor Speaker",
          //   icon: "/assets/icons/products/tech_aud_6.png"
          // }
        ]
      }
    }
  };
  
  export default officeEquipmentConfig;